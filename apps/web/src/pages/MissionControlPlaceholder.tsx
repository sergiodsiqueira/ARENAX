...
export function MissionControlPlaceholder() {
  ...
  return <AppShell user={userQuery.data}>
    <main className="mx-auto max-w-7xl px-5 py-7 sm:px-8 sm:py-10">
      ...
      <button type="button" aria-pressed={spaceStatusFilter === "in_progress"} onClick={() => setSpaceStatusFilter("in_progress")} className={`mt-8 w-full cursor-pointer rounded-2xl border p-5 text-left transition-shadow hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${attentionCount ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}><div className="flex items-start gap-3">{attentionCount ? <AlertTriangle className="mt-0.5 text-amber-600" size={21} /> : <CheckCircle2 className="mt-0.5 text-emerald-600" size={21} />}<div><p className="font-semibold">{attentionCount ? `${attentionCount} Espaço${attentionCount > 1 ? "s precisam" : " precisa"} de atenção` : "Arena operando normalmente"}</p><p className="mt-1 text-sm text-slate-600">{activeCount} {activeCount === 1 ? "Sessão" : "Sessões"} em andamento agora.</p></div></div></button>
