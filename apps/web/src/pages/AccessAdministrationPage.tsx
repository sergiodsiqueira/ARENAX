...
export function AccessAdministrationPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { setForm({ name: "", email: "", password: "", role: "operador" }); setFeedback("Usuário cadastrado com sucesso."); refresh(); },
    onError: (error) => setFeedback(error instanceof Error ? error.message : "Não foi possível cadastrar."),
  });
  const update = useMutation({
    ...
    onSuccess: () => { setFeedback("Acesso atualizado."); refresh(); },
    onError: (error) => setFeedback(error instanceof Error ? error.message : "Não foi possível atualizar."),
  });
  const reset = useMutation({
    mutationFn: ({ id, password }: { id: string; password: string }) => resetUserPassword(id, password),
    onSuccess: () => { setFeedback("Senha redefinida e acessos anteriores revogados."); setResetTarget(null); setNewPassword(""); },
    onError: (error) => setFeedback(error instanceof Error ? error.message : "Não foi possível redefinir a senha."),
  });

  const submit = (event: FormEvent) => { event.preventDefault(); setFeedback(null); create.mutate(form); };
  ...
  return <AppShell user={currentUser.data}>
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      {feedback && <div className="mt-6 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" role="status">{feedback}</div>}
      ...
      <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="mb-5 flex items-center gap-3"><span className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><UserPlus size={20} /></span><div><h2 className="font-semibold">Novo Usuário</h2><p className="text-sm text-slate-500">A senha inicial deve ter pelo menos 12 caracteres.</p></div></div>
        <form className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1.2fr_1.4fr_1fr_1fr_auto] xl:items-end" onSubmit={submit}>
          <label className="text-sm font-semibold text-slate-600">Nome<input className="admin-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label>
          <label className="text-sm font-semibold text-slate-600">E-mail<input className="admin-input" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></label>
          <label className="text-sm font-semibold text-slate-600">Senha inicial<input className="admin-input" type="password" minLength={12} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required /></label>
          <label className="text-sm font-semibold text-slate-600">Papel<select className="admin-input" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as ManagedUser["role"] })}>{roleOptions.map((role) => <option key={role} value={role}>{roleLabels[role as ManagedUser["role"]]}</option>)}</select></label>
