...
import { Activity, Camera, CircleAlert, Database, HardDrive, Radio, RefreshCw, Server, Video } from "lucide-react";
  ...
  return <AppShell user={user.data}>
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      {health.data && <>
        ...
        <section className="mt-8"><h2 className="text-lg font-semibold">Serviços e infraestrutura</h2><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{health.data.services.map((service) => { const Icon = icons[service.name as keyof typeof icons] ?? Server; return <article key={service.name} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><span className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><Icon size={21} /></span><StatusBadge status={service.status} /></div><h3 className="mt-4 font-semibold">{service.label}</h3><p className="mt-1 min-h-5 text-xs text-slate-500">{service.detail ?? formatDate(service.checked_at)}</p>{service.name === "storage" && <div className="mt-4"><div className="mb-2 flex justify-between text-xs text-slate-500"><span>{service.used_percent ?? 0}% utilizado</span><span>{formatBytes(service.free_bytes)} livres</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${(service.used_percent ?? 0) >= 90 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${Math.min(service.used_percent ?? 0, 100)}%` }} /></div></div>}{service.response_time_ms !== null && <p className="mt-3 text-xs text-slate-400">Resposta em {service.response_time_ms} ms</p>}</article>; })}</div></section>
