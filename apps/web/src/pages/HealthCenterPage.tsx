  ...
  return <AppShell user={user.data}>
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10">
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div><p className="text-sm font-semibold text-emerald-700">OPERAÇÃO</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Health Center</h1><p className="mt-2 text-slate-500">Saúde dos serviços e equipamentos que sustentam a operação da Arena.</p></div>
      ...
      {health.data && <>
        ...
        <section className="mt-8"><div className="flex items-end justify-between"><div><h2 className="text-lg font-semibold">Câmeras por Espaço</h2><p className="mt-1 text-sm text-slate-500">O sinal indica se a captura contínua está disponível para gerar Replays.</p></div><span className="text-sm text-slate-500">{activeCameras.length} ativa{activeCameras.length !== 1 ? "s" : ""}</span></div><div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{health.data.cameras.map((camera) => <div key={camera.camera_id} className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 last:border-0 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><button type="button" className="cursor-pointer rounded-xl bg-slate-50 p-2.5 text-emerald-700 hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => setLiveCamera(camera)} aria-label={`Abrir transmissão ao vivo de ${camera.description || camera.external_id}`} title="Abrir Câmera ao vivo"><Camera size={20} /></button><div><p className="font-semibold">{camera.description || camera.external_id}</p><p className="text-xs text-slate-500">{camera.space_name} · {camera.detail ?? formatDate(camera.checked_at)}</p></div></div><StatusBadge status={camera.status} /></div>)}{health.data.cameras.length === 0 && <p className="p-10 text-center text-slate-500">Nenhuma Câmera cadastrada.</p>}</div></section>
