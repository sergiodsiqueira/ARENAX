import { Activity, CalendarDays, Cpu, LayoutDashboard, MapPin, Menu, Power, Settings, ShieldUser, Users, WalletCards, X } from "lucide-react";
...
export function AppShell({ user, children }: AppShellProps) {
  ...
  const sidebar = <>
    ...
    <nav className="flex min-h-0 flex-1 flex-col overflow-y-auto px-4 py-2" aria-label="Navegação principal">
      ...
      {user?.role !== "operador" && <><p className="px-3 pb-1 pt-4 text-[.68rem] font-semibold tracking-[.16em] text-muted-foreground uppercase">Administração</p><NavLink className={({ isActive }) => `sidebar-link ${isActive ? "sidebar-link-active" : ""}`} to="/administracao/financeiro" onClick={() => setOpen(false)}><WalletCards size={19} /> Financeiro</NavLink><NavLink className={({ isActive }) => `sidebar-link ${isActive ? "sidebar-link-active" : ""}`} to="/administracao/equipamentos" onClick={() => setOpen(false)}><Cpu size={19} /> Equipamentos</NavLink><NavLink className={({ isActive }) => `sidebar-link ${isActive ? "sidebar-link-active" : ""}`} to="/administracao/acessos" onClick={() => setOpen(false)}><ShieldUser size={19} /> Acessos</NavLink><NavLink className={({ isActive }) => `sidebar-link ${isActive ? "sidebar-link-active" : ""}`} to="/administracao/configuracoes" onClick={() => setOpen(false)}><Settings size={19} /> Configurações</NavLink></>}
    </nav>
